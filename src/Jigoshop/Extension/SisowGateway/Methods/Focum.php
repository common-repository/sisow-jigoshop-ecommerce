...
class Focum extends AbstractSisow implements Method2, Processable
{
	const ID = 'sisow_focum';
	...
	{
		return __('Focum Achteraf Betalen', 'jigoshop2-sisow');
	}
	...
	{		
		...
		echo '<div class="row clearfix">';
		echo '<div class="form-group col-lg-6 clearfix">';
		echo '<labelfor="' . $this->getId() . '-iban">' . __('IBAN', 'jigoshop2-sisow') . '</label>';
		echo '<input type="text" class="form-control" id="' . $this->getId() . '-iban" name="' . $this->getId() . '_iban" value="">';
		echo '</div>';
		echo '</div>';
	...
	public function validateFields()
	{
		...
		{			
			throw new Exception(__('Please enter your IBAN', 'jigoshop2-sisow'));
			return false;
		}
